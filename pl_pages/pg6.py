...
def crypto_chart(crypto_item):
    ...
    return go.Candlestick(x=mexc_klines_df[M:]['opendate'].dt.time.astype(str).str[:-3], 
        open=mexc_klines_df[M:]['open'], high=mexc_klines_df[M:]['high'],
        low=mexc_klines_df[M:]['low'], close=mexc_klines_df[M:]['close'])

# ----- Mexc Candles indicator graph -----
def crypto_chart_ind(crypto_item): 

    mexc_klines_df = crypto_candles_df(crypto_item,1,150)
    return go.Scatter(x=mexc_klines_df['opendate'][M:].dt.time.astype(str).str[:-3],  
        y=crypto_ema_df(crypto_item)[M:], name="EMA_50", line=dict(color="blue"))
